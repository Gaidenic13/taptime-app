...
// Quick-add a scan-only member: just a name — no email, no password, no
// code. They scan the tag, ask to link their phone, and the admin approves.
export async function quickAddMember(actor, { first_name, last_name }) {
  ...
  `, actor.organization_id, first_name.trim(), (last_name || "").trim(), actor.location_id, actor.id);
